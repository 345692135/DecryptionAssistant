...
- (void)commandResponse:(SystemCommand *)data
{
    ...
    //0420，Log输出调试
    NSString* m = [data getModuleid];//模块码 400
    NSString* opCode = [data getOpcode];//操作码 404
    NSLog(@"MOduleid = %@, log upload response opCode = %@",m,opCode);
    if ([opCode isEqualToString:@"806"]) {
        NSLog(@"login log upload succeed");
    }
    if ([opCode isEqualToString:@"808"]) {
        NSLog(@"send email log upload succeed");
    }
    ...
    NSArray* actionResultInfoNodesArr = [doc nodesForXPath:@"/DATA/dwTypeId1" error:nil];
    ...
    NSLog(@"result:%@",actionResultInfoNodesArr);
...
-(SystemCommand*)createInitCommand:(NSArray*)log withType:(NSString*)type;
{
    ...
    NSString* logDataInfo = [self getLogDataInfo:log withType:type];
...
- (NSString*)getOpCode:(NSString*)type
{
    ...
    if ([type isEqualToString:@"log_mob_login"]) {
        opCode = [NSString stringWithFormat:@"803"];
    }
    if ([type isEqualToString:@"log_mob_email"]) {
        opCode = [NSString stringWithFormat:@"803"];
    }
    if ([type isEqualToString:@"log_mob_approve"]) {
        opCode = [NSString stringWithFormat:@"803"];
    }
    return opCode;
}

- (NSString*)getLogDataInfo:(NSArray*)logInfo withType:(NSString*)type
{
    id<IConfig> config = [[ConfigManager getInstance]getConifgPrivder];
    NSString* guid_1= [config getValueByKey:WSConfigItemGuid];//通过uuid计算得到
    NSString* sid_2 = [self getActiveAccountSID];
    NSString* osUserName_3 = @"ios";
    NSString* clientIP_4 = [self getIPAddress];//@"92.168.155.209";
    NSLog(@"clientIP_4: %@",clientIP_4);
    if (guid_1 == nil || sid_2 == nil) {
