...
+ (id)sharedManager 
{
	@synchronized(self) {
        if (!sharedSQLiteManager) {
            sharedSQLiteManager = [NSAllocateObject([self class], 0, NULL) init];
        }
    }
...
-(sqlite3 *)database
{
	...
	{
		first = NO;
		if (!sqlite3_open([[self getDatabaseFilepath] UTF8String], &database) == SQLITE_OK)
		{
		...
		{
            // Modify by Mr.cooriyou 2012-7-30 for encrypt database get DB key;
            
            NSString* key = [DBKeyInterface getDBPasswordByName:@"sys"];
            sqlite3_key( database, [key UTF8String], 16);
            // Modify end;
