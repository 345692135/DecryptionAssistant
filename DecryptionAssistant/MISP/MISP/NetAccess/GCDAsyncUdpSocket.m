...
- (id)initWithData:(NSData *)d timeout:(NSTimeInterval)t tag:(long)i
{
	if ((self = [super init]))
	{
		buffer = [d retain];//ggg-20150511 use retain for non-ARC
		timeout = t;
		tag = i;
...
-(void)dealloc
{
    ...
    if (resolvedAddresses) {
        [resolvedAddresses release];
    }
    if (resolveError) {
        [resolveError release];
    }
    if (address) {
        [address release];
    }
...
- (BOOL)connectToHost:(NSString *)host onPort:(uint16_t)port error:(NSError **)errPtr
{
	...
	dispatch_block_t block = ^{ @autoreleasepool {
		...
		[self asyncResolveHost:host port:port withCompletionBlock:^(NSArray *addresses, NSError *error) {
			...
			packet->addresses = [addresses retain];//ggg-20150511 use retain for non-ARC
		...
		else
		{
			...
			else
			{
				...
				if (result)
				{
					...
					cachedConnectedAddress = [address retain];//20150511
	...
	[self asyncResolveHost:host port:port withCompletionBlock:^(NSArray *addresses, NSError *error)
     {
		...
		packet->resolvedAddresses = [addresses retain];//ggg-20150511 use retain for non-ARC
		
         packet->resolveError = [error retain];
...
- (void)sendData:(NSData *)data toAddress:(NSData *)remoteAddr withTimeout:(NSTimeInterval)timeout tag:(long)tag
{
    ...
    [packet->address retain];//ggg-20150511 use retain for non-ARC
...
- (void)doPreSend
{
	...
	if (flags & kDidConnect)
	{
		...
		{
			currentSend->address = [cachedConnectedAddress retain];//ggg-20150511 use retain for non-ARC
	...
	else
	{
		...
		else if (currentSend->address == nil)
		{
			...
			else
			{
                ...
                [currentSend->address retain];//ggg-20150511 use retain for non-ARC
...
- (void)doSend
{
	...
	{
		// Connected socket
		const void *buffer = (Byte *)[currentSend->buffer bytes];
