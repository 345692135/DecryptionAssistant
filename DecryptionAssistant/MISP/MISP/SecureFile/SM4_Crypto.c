...
/****************************************************************************
*			Symetric cipher and digest function registrars FOR SW        	*
*****************************************************************************/
							...
							const unsigned char *in, size_t inl)
{
	...
	while(iLen < inl)
	{
		SMS4Crypt((unsigned char*)in+iLen,out+iLen,((unsigned int*)GetEncData(ctx)->rk));
		iLen += KEYLEN_sm4;
	}
	return 1;
}
							  ...
							  const unsigned char *iv, int enc)
{
	...
	if (enc)
	{
		SMS4KeyExt(GetEncData(ctx)->key, (unsigned int*)GetEncData(ctx)->rk, 0);   
	}
	else
	{
		SMS4KeyExt(GetEncData(ctx)->key, (unsigned int*)GetEncData(ctx)->rk, 1);   
	}
	return 1;
}
...
#define ByteSub(_AA) (Sbox[(_AA) >> 24 & 0xFF] << 24 ^ Sbox[(_AA) >> 16 & 0xFF] << 16 ^ Sbox[(_AA) >>  8 & 0xFF] <<  8 ^ Sbox[(_AA) & 0xFF])
...
{
     muint32 r = 0, mid= 0, x0= 0, x1= 0, x2= 0, x3= 0, *p = 0;   
     p = (muint32 *)Input;   
     x0 = p[0];   
     x1 = p[1];   
     x2 = p[2];   
