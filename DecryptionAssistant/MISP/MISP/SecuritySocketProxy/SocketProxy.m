...
static void AcceptCallBack(CFSocketRef socket, CFSocketCallBackType type, CFDataRef address, const void *data, void *info) {
    ...
    //begin forward
    SocketProxy* sp = (SocketProxy*)info;
    //printf("ForwardOperation Create to ip:<%s> prot:<%d>\r",[[sp remoteIp] UTF8String],[sp remotePort]);
    //    ForwardOperation* forwardOperation = [[ForwardOperation alloc]initWithHandle:*(CFSocketNativeHandle *) data];
    
    
    
    ForwardOperation* forwardOperation = [[ForwardOperation alloc]initWithHandle:*(CFSocketNativeHandle *) data remoteIp:[sp remoteIp] remotePort:[sp remotePort]];
    [sp.queue addOperation:forwardOperation];
    
    ///printf("ForwardOperation OK\r");
    [forwardOperation release];
    
    NSLog(@"队列中操作的个数是：%ld",(unsigned long)sp.queue.operationCount);
...
//  char punchline[] = "Proxy";
    ...
    NSLog(@"Socket proxy is working on port <%ld>",(long)port);
...
- (void)dealloc
{
    ...
    [remoteIp release];
     remoteIp = nil;
    NSLog(@"Socket proxy is not working on port <%ld>",(long)remotePort);
    [super dealloc];
}
